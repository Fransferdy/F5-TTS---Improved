...
def best_similarity(generated_text: str, ref_text_1: str, ref_text_2: str) -> float:
    """
    Compare generated_text to both references and return the highest score.
    """
    score1 = similarity_score(generated_text, ref_text_1)
    score2 = similarity_score(generated_text, ref_text_2)
    return max(score1, score2)
def numbers_to_words_dual(text: str):
    """
    Returns:
      normal_text: all numbers converted normally using num2words
      split_text:  numbers > 999 split into two parts before conversion
    """
...
def raw_infer(textv1,textv2,audio,sr, ref_text,gen_text_batches, exp_name, remove_silence, cross_fade_duration=0.15):
    ...
    gr.Info(f"Generated audio text: {result_text} ")
    print(f"Generated audio text: {result_text} ")
    
    adherenceScore = best_similarity(result_text, textv1, textv2)
    gr.Info(f"TTS Text Adherence: {adherenceScore} ")
    print("ADherence Score: ", adherenceScore)
    return (target_sample_rate, final_wave), spectrogram_path, adherenceScore
